  ...
  if file.open ( "page.tmpl" ) then
    ...
    if file.open ( _filename, "w" ) then
      ...
      for part in string.gmatch ( tmpl,  "([^%|]+)" ) do
        ...
        elseif part == "NODE_NAME"   then file.write ( _node_name )
        elseif part == "MENU_LINKS"  then file.write ( _links )
...
for k,v in pairs ( framework ) do
  if v[1] and v[2] and v[3] then
    ...
    for kL,vL in pairs ( framework ) do
      if k == kL then
        links = links .. '<a href="/' .. vL[1] .. '" class="cur">' .. vL[2] .. '</a>'
      else
        links = links .. '<a href="/' .. vL[1] .. '">' .. vL[2] .. '</a>'
      end
    end
